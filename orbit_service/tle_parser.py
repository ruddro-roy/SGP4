...
class TLEParser:
    ...
    def __init__(self):
        self.line1_pattern = re.compile(
            r"^1\s(\d{5})[A-Z]\s{2}(\d{2})(\d{3}\.\d{8})\s(.{10})\s(\d)\s(\d{5})\s(.{1})\s(\d{10})\s(.{1})\s(\d{10})\s(.{1})\s(\d{10})\s(.{5})\s(\d{4})([+-]\d{4})"
        )
        self.line2_pattern = re.compile(
            r"^2\s(\d{5})\s(\d{8})\s(\d{8})\s(\d{7})\s(\d{8})\s(\d{8})\s(\d{11})\s(\d{5})"
        )

    def calculate_nodal_precession(
        self,
        ...
        nodal_precession = (
            -1.5 * J2 * (EARTH_RADIUS_KM / p) ** 2 * n0 * math.cos(inclination_rad)
        )
    ...
    def calculate_gmst(self, julian_date: float) -> float:
        ...
        gmst_hours = (
            18.697374558 + 0.06570982441908 * d + 1.00273790935 * h + 0.000026 * t * t
        )
    ...
    def tle_data_to_lines(self, tle_data: Dict[str, Any]) -> Tuple[str, str]:
        ...
        if "line1" in tle_data and "line2" in tle_data:
            ...
            original_bstar_parsed = tle_data.get(
    ...
    def teme_to_ecef(self, r_teme: np.ndarray, epoch_datetime: datetime) -> np.ndarray:
        ...
        rotation_matrix = np.array(
            [[cos_gmst, sin_gmst, 0], [-sin_gmst, cos_gmst, 0], [0, 0, 1]]
        )
    ...
    def propagate_orbit(
        self, tle_data: Dict[str, Any], tsince_minutes: float
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "gmst_hours": self.calculate_gmst(
                self.datetime_to_julian_date(current_time)
            ),
            "anomalies": {
    ...
    def parse_tle(
        self, line1: str, line2: str, name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        tle_data["nodal_precession"] = self.calculate_nodal_precession(
            tle_data["inclination_deg"],
            tle_data["eccentricity"],
            tle_data["mean_motion_rev_per_day"],
        )
    ...
    def _parse_line1(self, line1: str) -> Dict[str, Any]:
        ...
        epoch_datetime = datetime(full_year, 1, 1, tzinfo=timezone.utc) + timedelta(
            days=epoch_day - 1
        )
