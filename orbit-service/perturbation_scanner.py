...
class PerturbationAlert:
    ...
    def __init__(self, timestamp: datetime, satellite_id: int, 
                 deviation_km: float, level: PerturbationLevel,
                 predicted_pos: np.ndarray, observed_pos: np.ndarray):
...
class PerturbationScanner:
    ...
    def scan_satellite_perturbations(self, satellite_data: Dict[str, Any],
                                   predicted_positions: List[Tuple[datetime, np.ndarray]],
                                   observed_positions: List[Tuple[datetime, np.ndarray]]) -> List[PerturbationAlert]:
        """
        ...
        for pred_time, pred_pos in predicted_positions:
            ...
            closest_obs = min(observed_positions, 
            ...
            if deviation_km > self.error_threshold_km:
                ...
                logger.warning(f"Perturbation detected for satellite {satellite_data.get('name', 'Unknown')}: "
