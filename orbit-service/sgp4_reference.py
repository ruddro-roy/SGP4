...
class SGP4Propagator:
    ...
    def parse_tle(self, line1, line2):
        try:
            ...
            self.epochyr = 1900 + self.epochyr if self.epochyr >= 57 else 2000 + self.epochyr
    ...
    def sgp4init(self):
        ...
        ao = ao * (1.0 - delta1/3.0 - delta1*delta1 - 134.0*delta1*delta1*delta1/81.0)
        ...
        self.argdot = 1.5 * J2 * (RE / self.p)**2 * (4.0 - 5.0 * self.sinio * self.sinio) * self.no / 2.0
        
        # mdot = no + (3/2) * j2 * (re / p)^2 * sqrt(1-e^2) * (3cos(i)^2 -1) / (2 * (1-e^2))
        self.mdot = self.no + 1.5 * J2 * (RE / self.p)**2 * self.betao * self.x3thm1 / (2.0 * self.betao2)
    ...
    def propagate(self, tsince):
        ...
        for iter in range(10):
            ...
            delta_ep = (u - ayn * cosep + axn * sinep - ep) / (1.0 - axn * cosep - ayn * sinep)
        ...
        # SPP corrections
        rk = r * (1.0 - 1.5 * J2 * (RE / pl)**2 * self.x3thm1) + \
